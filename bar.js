...
/** Convert locale‑messy cell → Number (handles commas, currency, spaces) */
const toNum = v => {
...
async function loadBarData(){
  ...
  const inventory = invRaw.map(r => {
    ...
    return {
      name : r.Name,
      abv,
      pricePerMl : toNum(r['Cena/L']) / 1000            // may be 0 if blank
    };
  }).filter(i => i.name);   // keep row even if pricePerMl is 0
  ...
  const drinks = Object.fromEntries(drinkNames.map(n => [n, {
    name: n, volumeMl: 0, alcPercent: 0, price: 0, parts: []
  }]));
...
function renderList(drinks, shots){
  ...
  if (Object.keys(drinks).length === 0){
    app.insertAdjacentHTML('beforeend', '<p class="err">No drinks generated – check your Google Sheet contents.</p>');
    return;
  }
...
function renderDetail(d){
  ...
  app.insertAdjacentHTML('beforeend', `
    <p>Volume: <strong>${d.volumeMl} ml</strong><br>
